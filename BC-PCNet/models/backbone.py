...
class KPFCN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.coarse_out = nn.Conv1d(in_dim//2, config.coarse_feature_dim,  kernel_size=1, bias=True) #卷积层  输入以及输出维度，卷积核大小
    ...
    def forward(self, batch, phase = 'encode'):
        # Get input features   #获取输入特征

        if phase == 'coarse' :

            x = batch['features'].clone().detach()
            # 1. joint encoder part   #编码层
            self.skip_x = []
            for block_i, block_op in enumerate(self.encoder_blocks):     #编码层 第一层SimpleBlock，第二层ResnetBottleneckBlock
                if block_i in self.encoder_skips:
                    self.skip_x.append(x)
                x = block_op(x, batch)  # [N,C]     #最后输出713，2048
            #解码层
            for block_i, block_op in enumerate(self.decoder_blocks):     #解码层
                if block_i in self.decoder_concats:
                    x = torch.cat([x, self.skip_x.pop()], dim=1)
                x = block_op(x, batch)   #2621*1024
                if block_i == 1 :
                    coarse_feats = x.transpose(0,1).unsqueeze(0)  #[B, C, N]   #1*1024*2621
                    coarse_feats = self.coarse_out(coarse_feats)  #[B, C, N]  卷积    1*528*2621
                    coarse_feats = coarse_feats.transpose(1,2).squeeze(0)     #变格式   2621*528
