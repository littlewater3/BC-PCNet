...
class Matching(nn.Module):
    ...
    def get_match( conf_matrix, thr, mutual=True):
        ...
        if mutual:
            mask = mask \
                   ...
                   * (conf_matrix == conf_matrix.max(dim=1, keepdim=True)[0])     #mask修改，判断横竖是否都是最大值，都是最大值才标记为1

        #find all valid coarse matches  找到所有有效的粗匹配
        index = (mask==True).nonzero()   #把true的索引提取出来
    ...
    def forward(self, src_feats, tgt_feats, src_pe, tgt_pe, src_mask, tgt_mask, data, pe_type="rotary"):
        ...
        @return:
        '''
        src_feats = self.src_proj(src_feats)    #将特征进行528*528矩阵变换
        ...
        if not self.entangled :
            src_feats = VolPE.embed_pos(pe_type, src_feats, src_pe)    #将位置编码信息融入特征中
        ...
        if self.match_type == "dual_softmax":
            ...
            sim_matrix = torch.einsum("bsc,btc->bst", src_feats, tgt_feats)   #1238*1383  方阵
            ...
            assign_matrix = log_assign_matrix.exp()   #返回指数
            conf_matrix = assign_matrix[:, :-1, :-1].contiguous()

        coarse_match, _, _ = self.get_match(conf_matrix, self.confidence_threshold)   #返回的索引
