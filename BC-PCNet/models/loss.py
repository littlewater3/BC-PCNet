...
class MatchMotionLoss(nn.Module):
    def __init__(self, config):
        ...
        self.cro_w0 = config['crossentropy_weight0']
        self.cro_w1 = config['crossentropy_weight1']
        self.cro_w2 = config['crossentropy_weight2']
        self.cro_w3 = config['crossentropy_weight3']
    ...
    def ge_coarse_loss(self, data, loss_info, eval_metric=False):
        ...
        src_mask = data['src_mask']    #1*1238
        tgt_mask = data['tgt_mask']    #1*1383
        conf_matrix_pred = data['conf_matrix_pred']   #评分矩阵  1*1238*1383
        match_gt = data['coarse_matches']   #张量  2*718   对应关系
        R_s2t_gt = data['batched_rot']     #1*3*3
        t_s2t_gt = data['batched_trn']     #1*3*1
        criteon = nn.CrossEntropyLoss()
        #get the overlap mask, for dense motion loss 获取重叠部分标签，用于密集运动损失
        s_overlap_mask = torch.zeros_like(src_mask).bool()
        for bi, corr in enumerate (match_gt):
            s_overlap_mask[bi][ corr[0] ] = True
        # compute focal loss    第二个TMP层的loss
        c_weight = (src_mask[:, :, None] * tgt_mask[:, None, :]).float()   #1*1238*1383
        conf_matrix_gt = self.match_2_conf_matrix(match_gt, conf_matrix_pred)  #根据预测值（预测置信度矩阵）的格式，转化真实值
        ...
        A = data['src_feat_mask']
        B = data['src_mask_region']
        loss_cross_0_src = criteon(data['src_feat_mask'][0],data['src_mask_region'])
        loss_cross_0_tgt = criteon(data['tgt_feat_mask'][0],data['tgt_mask_region'])
        loss_cross_1_src = criteon(data['src_feat_mask'][1],data['src_mask_region'])
        loss_cross_1_tgt = criteon(data['tgt_feat_mask'][1],data['tgt_mask_region'])
        loss_cross_2_src = criteon(data['src_feat_mask'][2],data['src_mask_region'])
        loss_cross_2_tgt = criteon(data['tgt_feat_mask'][2],data['tgt_mask_region'])
        loss_cross_3_src = criteon(data['src_feat_mask'][3],data['src_mask_region'])
        loss_cross_3_tgt = criteon(data['tgt_feat_mask'][3],data['tgt_mask_region'])

        loss = loss + self.mat_w * focal_coarse + self.cro_w0 * (loss_cross_0_src + loss_cross_0_tgt) \
                + self.cro_w1 * (loss_cross_1_src + loss_cross_1_tgt) + self.cro_w2 * (loss_cross_2_src + loss_cross_2_tgt) \
                + self.cro_w3 * (loss_cross_3_src + loss_cross_3_tgt)
        ...
        if self.positioning_type == "procrustes":    #procrustes

            for layer_ind in data["position_layers"]:
                # compute focal loss     #第一个TMP层中的loss
    ...
    def compute_correspondence_loss(self, conf, conf_gt, weight=None):
        '''
        @param conf: [B, L, S]    预测
        @param conf_gt: [B, L, S]    真实值
        @param weight: [B, L, S]
        @return:
        '''
        pos_mask = conf_gt == 1      #confindence为置信度    positive
        neg_mask = conf_gt == 0      #negative
        ...
        # focal loss
        conf = torch.clamp(conf, 1e-6, 1 - 1e-6)    #更改预测值的数据范围
        ...
        if self.match_type == "dual_softmax":
            ...
            loss_pos = - alpha * torch.pow(1 - conf[pos_mask], gamma) * (conf[pos_mask]).log()   #pow为平方  Matching Loss  718
            loss_neg = - alpha * torch.pow(conf[neg_mask], gamma) * (1 - conf[neg_mask]).log()   #[1711436]
            loss = pos_w * loss_pos.mean() + neg_w * loss_neg.mean()     #正向以及反向都求平均值
            return loss    #loss为matching loss
