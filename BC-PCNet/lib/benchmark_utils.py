...
#获取点对应关系    search_voxel_size为overlap_radius，0.0375
...
def KDTree_corr ( src_pcd_transformed, tgt_pcd, search_voxel_size, K=None):
    ...
    for i, point in enumerate(src_pcd_transformed.points):
        [count, idx, _] = pcd_tree.search_radius_vector_3d(point, search_voxel_size)  #查询所有和查询点距离小于给定半径的点。
        if K is not None:
            idx = idx[:K]
        for j in idx:
            correspondences.append([i, j])
    #print(type(correspondences),correspondences)
    correspondences = np.array(correspondences)   #每个行转换为一整个数组，104998*2
    #print(correspondences.shape,type(correspondences),correspondences)   
    return correspondences
