...
class Trainer(object):
    ...
    def _snapshot(self, epoch, name=None):   #保存信息
        state = {
            'epoch': epoch,
    ...
    def _load_pretrain(self, resume):     #读模型
        print ("loading pretrained", resume)
        if os.path.isfile(resume):
            state = torch.load(resume)
            self.model.load_state_dict(state['state_dict'])
            self.start_epoch = state['epoch']      #读取中断的epoch
    ...
    def _get_lr(self, group=0):       #读学习率
        return self.optimizer.param_groups[group]['lr']


    def inference_one_batch(self, inputs, phase):      #一个batch流程
        ...
        if (phase == 'train'):      #训练流程
            ...
            data = self.model(inputs, timers=self.timers)  # [N1, C1], [N2, C2]    输入input，载入到模型中，输出data,此处调用forward
            ...
            loss_info['loss'].backward()       #反向传播计算梯度,
    ...
    def inference_one_epoch(self, epoch, phase):     #一个epoch训练流程
        gc.collect()
        assert phase in ['train', 'val', 'test']
        ...
        num_iter = int(len(self.loader[phase].dataset) // self.loader[phase].batch_size) # drop last incomplete batch  //为向下取整,  数据集的长度/batch_size
        c_loader_iter = self.loader[phase].__iter__()    #读取数据

        self.optimizer.zero_grad()   #梯度置为0
        for c_iter in tqdm(range(num_iter)):  # loop through this epoch   一个epoch中循环多少个batch    tqdm增加进度条信息
            ...
            for k, v in inputs.items():    #条目,从dataloader返回的字典中读取
                if type(v) == list:
            ...
            #print('第' + str(c_iter+1) + "个batch")
            loss_info = self.inference_one_batch(inputs, phase)      #在循环中调用一个batch，输入数据字典以及选择的模式
            ...
            if ((c_iter + 1) % self.iter_size == 0 and phase == 'train'):
                gradient_valid = validate_gradient(self.model)     #验证梯度
                if (gradient_valid):
                    self.optimizer.step()     #step函数更新优化器的学习率
                else:
                    self.logger.write('gradient not valid\n')
                self.optimizer.zero_grad()     #zero_grad()清除累加
    ...
    def train(self):     #开始训练
        ...
        for epoch in range(self.start_epoch, self.max_epoch):     #epoch个循环训练
            with torch.autograd.set_detect_anomaly(True):     #正向传播时：开启自动求导的异常侦测
                if self.timers: self.timers.tic('run one epoch')
                #print('第' + str(epoch) + "个epoch")
                stats_meter = self.inference_one_epoch(epoch, 'train')   #执行一个epoch
