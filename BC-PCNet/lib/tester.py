...
class _3DMatchTester(Trainer):       #tester继承trainer
    """
    3DMatch tester
    """
    ...
    def test_thr(self, conf_threshold=None):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                ir = MML.compute_inlier_ratio(match_pred, data, inlier_thr=0.1).mean()    #IR指标

                rr1 = MML.compute_registration_recall(rot, trn, data, thr=0.2) # 0.2m     #RR指标
